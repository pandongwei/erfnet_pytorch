...
class Encoder_simplified(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        self.initial_block = DownsamplerBlock_simplified(3,16)

        self.layers = nn.ModuleList()

        self.layers.append(DownsamplerBlock_simplified(16,64))

        for x in range(0, 2):    # 5 times-> 2 times
           self.layers.append(non_bottleneck_1d(64, 0.03, 1))

        self.layers.append(DownsamplerBlock_simplified(64,128))
...
class Decoder_regression (nn.Module):
    ...
    def forward(self, input):
        ...
        # outputs = torch.squeeze(mean,dim=1)
        # outputs = mean

        return outputs

class Decoder_regression_simplified (nn.Module):
    def __init__(self, num_classes):
        super().__init__()
    ...
    def forward(self, input):
        ...
        # print(var.shape)
        log_var2 = var
        outputs = torch.cat([mean,log_var2],dim=1)
        # outputs = torch.squeeze(mean,dim=1)
        # outputs = mean
...
class ERFNet_regression_simplified(nn.Module):
    def __init__(self, num_classes=2, encoder=None):  #use encoder to pass pretrained encoder
        super().__init__()

        if (encoder == None):
            self.encoder = Encoder_simplified(num_classes)
        else:
            self.encoder = encoder
        self.decoder = Decoder_regression_simplified(num_classes)
